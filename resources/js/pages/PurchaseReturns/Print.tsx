...
export default function Print() {
    ...
    const downloadPDF = async () => {
        ...
        if (printContent) {
            const opt = {
                margin: 0.25,
                filename: `purchase-return-${returnData.return_number}.pdf`,
    ...
    return (
        <ReportShell>
            <Head title={`Devolução de compra #${returnData.return_number}`} />
            ...
            <div className="document-print-container space-y-6">
                <ReportHero
                    title={willCreateDebitNote ? 'Nota de Débito' : 'Nota / Guia de Devolução'}
                    ...
                    statusPills={[
                        ...
                        { label: willCreateDebitNote ? 'Gera nota de débito' : 'A aguardar aprovação', tone: willCreateDebitNote ? 'success' : 'warning' },
                    ]}
                    meta={[
                    ...
                    note={willCreateDebitNote ? 'Impacto no stock: devolução física registada.' : 'Este documento pode ser convertido em nota de débito após aprovação.'}
                />

                <div className="grid gap-6 lg:grid-cols-2">
                    <ReportCard title="Fornecedor" subtitle="Dados da devolução">
                        <ReportKeyValueGrid
                            columns={2}
                            items={[
                                { label: 'Nome', value: vendor.company_name || vendor.name || returnData.vendor?.name || '-' },
                                { label: vendorTaxLabel || 'NUIT', value: vendor.tax_number || '-' },
                                { label: 'E-mail', value: vendor.email || '-' },
                                { label: 'Código', value: returnData.vendor_details?.vendor_code || '-' },
                    ...
                    <ReportCard title="Contexto operacional" subtitle="Regularização e referência">
                        <ReportKeyValueGrid
                            ...
                            items={[
                                ...
                                { label: 'Gera nota de débito', value: willCreateDebitNote ? 'Sim' : 'Não' },
                                { label: 'Motivo', value: formatReason(returnData.reason) },
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <ReportPill tone="info">Devolução de compra</ReportPill>
                    <ReportPill tone={documentStatus.tone}>{documentStatus.label}</ReportPill>
                    {willCreateDebitNote ? <ReportPill tone="success">Regulariza factura</ReportPill> : <ReportPill tone="warning">Pendência de aprovação</ReportPill>}
                </div>
            </div>
        </ReportShell>
    );
