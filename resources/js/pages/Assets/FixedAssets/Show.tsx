        ...
        <AuthenticatedLayout breadcrumbs={[{ label: t('Contabilidade') }, { label: t('Activos') }, { label: t('Activos Fixos'), url: route('sce.fixed-assets.index') }, { label: asset.asset_code }]}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                <Card className="bg-gradient-to-br from-green-50 to-emerald-50 border-green-200">
                    <CardContent className="p-6 text-center"><p className="text-xs text-green-600">{t('Valor Líquido')}</p><p className="text-2xl font-bold text-green-800">{fmt(asset.net_book_value)}</p><Badge className={`mt-2 border-0 ${asset.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{asset.status}</Badge></CardContent>
                </Card>
            </div>
