...
interface CreditNote {
    ...
    reason: string;
    notes?: string;
    sales_return?: { return_number: string };
    items: Array<{
        id: number;
...
export default function Print() {
        ...
        if (printContent) {
            try {
                await saveElementAsPdf(
                    ...
                    buildCommercialDocumentPdfOptions(`nota-credito-${sanitizeFilename(documentNumber)}.pdf`),
                );
    ...
    return (
        <div className="min-h-screen bg-slate-100 py-6 print:bg-white print:py-0">
            <Head title={`Nota de crédito #${documentNumber}`} />
            ...
            <CommercialDocumentTemplate
                ...
                recipient={{
                    title: 'Para',
                    name: counterparty.company_name || counterparty.name || note.customer?.name || 'Cliente',
                    ...
                    email: counterparty.email || note.customer?.email,
                }}
                statusPills={[
                    { label: statusLabel[note.status] || note.status || 'Rascunho', tone: note.status === 'cancelled' ? 'danger' : 'success' },
                    { label: 'IVA regularizado', tone: 'success' },
                ]}
                meta={[
                    { label: 'Data', value: formatDate(note.credit_note_date, page.props) },
                    { label: 'Factura original', value: note.invoice?.invoice_number || '-' },
                    { label: 'Devolução', value: note.sales_return?.return_number || '-' },
                ...
                totals={[
                    ...
                    { label: 'IVA regularizado', value: formatDocumentMoney(note.tax_amount, settings) },
                    { label: 'Total creditado', value: formatDocumentMoney(note.total_amount, settings), emphasis: true },
                ...
                observations={note.notes || [
                    ...
                    'A nota de crédito corrige valores previamente facturados.',
                    'IVA regularizado conforme documento original.',
                ].join('\n')}
