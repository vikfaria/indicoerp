...
export default function Print() {
    ...
    const customerDisplayName = data.customer.company_name || data.customer.name;
    const showCustomerPersonName = Boolean(data.customer.company_name && data.customer.company_name !== data.customer.name);
    const customerTaxLabel = data.customer.tax_label || t('Tax Number');
    ...
    const downloadPDF = async () => {
        ...
        if (printContent) {
            const opt = {
                margin: 0.25,
                filename: `customer-detail-${data.customer.name}-${filters.start_date || 'all'}.pdf`,
    ...
    return (
        <div className="min-h-screen bg-white">
            <Head title={t('Customer Detail Report')} />
            {isDownloading && (
            ...
            <div className="report-container bg-white max-w-7xl mx-auto p-8">
                <div className="border-b-2 border-gray-800 pb-6 mb-8">
                    <div className="flex justify-between items-start">
                        ...
                        <div className="text-right">
                            <h2 className="text-2xl font-bold text-gray-900 mb-3">{t('CUSTOMER DETAIL REPORT')}</h2>
                            <p className="text-sm text-gray-600">{t('Customer')}: {customerDisplayName}</p>
                            {showCustomerPersonName && (
                                <p className="text-sm text-gray-600">{data.customer.name}</p>
                            )}
                            {data.customer.tax_number && (
                                <p className="text-sm text-gray-600">{customerTaxLabel}: {data.customer.tax_number}</p>
                            )}
                ...
                <div className="mb-6">
                    <h3 className="text-lg font-bold border-b-2 border-black pb-2 mb-3">{t('SALES INVOICES')}</h3>
                ...
                <div className="mb-6">
                    <h3 className="text-lg font-bold border-b-2 border-black pb-2 mb-3">{t('SALES RETURNS')}</h3>
                    <table className="w-full border-collapse mb-4">
                        <thead>
                            <tr className="border-b-2 border-black">
                ...
                <div className="mb-6">
                    <h3 className="text-lg font-bold border-b-2 border-black pb-2 mb-3">{t('CREDIT NOTES')}</h3>
                    <table className="w-full border-collapse mb-4">
                        <thead>
                            <tr className="border-b-2 border-black">
                                <th className="text-left py-2 px-2 text-sm font-semibold">{t('Credit Note Number')}</th>
                                <th className="text-left py-2 px-2 text-sm font-semibold">{t('Date')}</th>
                                <th className="text-right py-2 px-2 text-sm font-semibold">{t('Total')}</th>
                        ...
                        <tbody>
                            {data.credit_notes.map((note: any, idx: number) => (
                                <tr key={idx} className="border-b border-gray-200">
                                    <td className="py-2 px-2 text-sm">{note.credit_note_number}</td>
                ...
                <div className="mb-6">
                    <h3 className="text-lg font-bold border-b-2 border-black pb-2 mb-3">{t('CUSTOMER PAYMENTS')}</h3>
                    <table className="w-full border-collapse mb-4">
                        <thead>
                            <tr className="border-b-2 border-black">
                ...
                <div className="border-t-2 border-black pt-4 mt-8">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div><span className="font-semibold">{t('Total Credit Notes')}:</span> {formatCurrency(data.summary.total_credit_notes)}</div>
