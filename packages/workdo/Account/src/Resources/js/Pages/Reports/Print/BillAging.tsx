                ...
                <div className="border-b-2 border-gray-800 pb-6 mb-8">
                    <div className="flex justify-between items-start">
                        ...
                        <div className="text-right">
                            <h2 className="text-2xl font-bold text-gray-900 mb-3">{t('BILL AGING REPORT')}</h2>
                    ...
                    <tbody>
                        {data.vendors.map((vendor: any, idx: number) => (
                            <tr key={idx} className="border-b border-gray-200">
                                ...
                                <td className="py-2 px-2 text-sm text-right">{formatCurrency(vendor.current)}</td>
                                <td className="py-2 px-2 text-sm text-right">{formatCurrency(vendor['1_30_days'])}</td>
                                <td className="py-2 px-2 text-sm text-right">{formatCurrency(vendor['31_60_days'])}</td>
                                <td className="py-2 px-2 text-sm text-right">{formatCurrency(vendor['61_90_days'])}</td>
                                <td className="py-2 px-2 text-sm text-right">{formatCurrency(vendor.over_90_days)}</td>
                                <td className="py-2 px-2 text-sm text-right font-semibold">{formatCurrency(vendor.total)}</td>
                            </tr>
                        ))}
