...
class SupporUtility extends Model
{
    ...
    public static function GivePermissionToRoles($role_id = null, $rolename = null)
    {
        ...
        if ($rolename == 'staff') {
            ...
            self::syncPermissionsByName($roles_v, $staff_permissions);
        }

        if ($rolename == 'client') {
            $roles_v = Role::where('name', 'client')->where('id', $role_id)->first();
            self::syncPermissionsByName($roles_v, $client_permissions);
        }

        if ($rolename == 'vendor') {
            $roles_v = Role::where('name', 'vendor')->where('id', $role_id)->first();
            self::syncPermissionsByName($roles_v, $vendor_permissions);
        }
    }
