...
export const paymentMethodBtn = (data?: any) => {
    ...
    const mollieEnabled = getAdminSetting('mollie_enabled');

    if (mollieEnabled === 'on') {
        return [{
            id: 'mollie-payment',
            dataUrl: route('payment.mollie.store'),
            ...
            component: (
                <div className="flex items-center space-x-3 p-3 border border-gray-200 dark:border-gray-700 rounded-lg w-full">
                    ...
                    <Label htmlFor="mollie" className="cursor-pointer flex items-center space-x-2">
                        <div>
                            <div className="font-medium text-gray-900 dark:text-white">{t('Mollie')}</div>
                        </div>
                        <img src={getPackageFavicon('Mollie')} alt="Mollie" className="h-10 w-10" />
                    </Label>
                </div>
            )
        }];
