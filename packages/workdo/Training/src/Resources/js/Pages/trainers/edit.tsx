...
export default function EditTrainer({ data: initialData, trainer, onSuccess, branches, departments }: EditProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('training.trainers.update', trainer.id), {
            onSuccess: () => {
                onSuccess();
            },
        });
    ...
    return (
        <DialogContent className="sm:max-w-2xl">
            <DialogHeader>
                <DialogTitle>{t('Edit Trainer')}</DialogTitle>
            </DialogHeader>
